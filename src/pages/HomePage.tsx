...
export const HomePage = () => {
      ...
      if (user) {
        ...
        if (isDemo) {
          ...
          const {
            data: matches,
            error
          } = await supabase.from('games').select('*').or(`player1_id.eq.${user.id},player2_id.eq.${user.id}`).order('created_at', {
            ascending: false
          }).limit(3);
          if (error) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('games').select('*').or(`player1_id.eq.${user.id},player2_id.eq.${user.id}`).eq('status', 'active').limit(1).maybeSingle();
      if (error) {
  ...
  return <div className="space-y-10">
      ...
      <section className="py-12 px-4 text-center relative">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {!user ? <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
              ...
              {activeGame ? <Link to={`/game/${activeGame.id}`}>
                  <Button size="lg" style={{ backgroundColor: '#e2c044' }} className="hover:opacity-80 text-black font-semibold animate-pulse">
                    Rejoin Active Game
                  </Button>
                </Link> : <>
      ...
      {user && recentMatches && recentMatches.length > 0 && <section className="py-10">
          <div className="max-w-6xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {recentMatches.map(match => <MatchCard key={match.id} match={match} showViewDetails={false} />)}
            </div>
          </div>
        </section>}
      
      {/* Checkers Board Demo */}
      {!user && <section className="py-10">
